...
$files = @(
  ...
  "backend/services/agent_skill_bindings.py",
  "backend/services/agent_skill_catalog.py",
  "backend/services/backend_module_boundaries.py",
  "backend/services/clickhouse_schema_catalog.py",
  "backend/services/data_center_catalog.py",
...
$pythonSmoke = @(
  ...
  "assert {'alphatrace_static_seed','mysql_system_config_store','clickhouse_business_store','bocha_web_search','future_professional_market_data'} <= provider_ids, 'data api provider catalog smoke failed'",
  ...
  "assert {'agent_runtime','runner_adapters','legacy_crypto_trading','tradingagents_external_runtime','langalpha_external_workbench','mysql_system_config_store','clickhouse_business_store'} <= boundary_ids, 'module boundary catalog smoke failed'",
  ...
  "connector_ids = {item['connector_id'] for item in data_center['connectors']}",
  "assert {'static_seed_connector','bocha_search_tool_ingestion','professional_market_data_connector','mysql_system_config_connector'} <= connector_ids, 'data center catalog smoke failed'",
  "skills = get_agent_skill_catalog().to_response()",
  "skill_ids = {item['skill_id'] for item in skills['skills']}",
  "assert {'market_context_skill','evidence_retrieval_skill','bull_bear_debate_skill','risk_review_skill','final_decision_skill'} <= skill_ids, 'agent skill catalog smoke failed'",
  "bindings = get_agent_skill_binding_catalog().to_response()",
  "binding_ids = {item['role_id'] for item in bindings['bindings']}",
  "assert {'market_analyst','bull_researcher','bear_researcher','risk_analyst','portfolio_manager'} <= binding_ids, 'agent skill binding catalog smoke failed'",
  "clickhouse = get_clickhouse_schema_catalog().to_response()",
  "clickhouse_tables = {item['table_name'] for item in clickhouse['tables']}",
  "assert {'alpha_trace_runtime_events','alpha_trace_agent_reports','alpha_trace_evidence_refs','alpha_trace_decisions','alpha_trace_market_facts'} <= clickhouse_tables, 'clickhouse schema catalog smoke failed'",
  "review = get_architecture_review_bundle()",
  "assert {'architecture','moduleBoundaries','externalComponents','integrationDecisions','dataCenter','skills','agentSkillBindings','clickHouseSchemas','readiness'} <= set(review.keys()), 'architecture review bundle smoke failed'",
