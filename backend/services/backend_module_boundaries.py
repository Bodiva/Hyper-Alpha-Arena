    ...
    return BackendModuleBoundaryCatalog(
        ...
        modules=(
            ...
            BackendModuleBoundary(
                ...
                depends_on=("data_api_catalog", "mysql_persistence"),
                extension_points=("professional market data providers", "external evidence providers", "file ingestion"),
                notes="Static seed remains a development fallback; MySQL is the target domain persistence.",
                migration_target="Migrate static/domain stores to MySQL-backed stores with JSON fallback.",
            ),
            BackendModuleBoundary(
                ...
                depends_on=("runner_adapters", "async_task_management", "mysql_persistence"),
                extension_points=("AgentRunStore", "RuntimeEvent stream", "AgentArtifactStore", "metrics snapshots"),
                notes="This layer owns product persistence even when an external runner has its own checkpoint.",
            ...
            BackendModuleBoundary(
                module_id="mysql_persistence",
                display_name="MySQL Product Persistence",
                boundary_type="infrastructure_target",
                ownership="platform_backend",
                status="active_with_json_fallback",
                directories=("backend/database", "backend/services/*_store"),
                depends_on=("runtime_config", "agent_runtime", "alphatrace_domain_services"),
                extension_points=("AgentRunStore", "domain stores", "system config", "async task snapshots", "artifact store"),
                notes="MySQL is the formal target; JSON remains a local/MVP fallback only.",
                migration_target="Converge all product-owned state to MySQL 8.0+ with JSON columns where useful.",
            ),
            BackendModuleBoundary(
        ...
        policies={
            ...
            "persistence_policy": "MySQL is the product persistence target; JSON fallback is local/MVP only.",
