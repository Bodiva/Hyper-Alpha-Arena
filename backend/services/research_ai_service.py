"""
Research AI Service - Main Agent for Full-Site AI Intelligence

Research AI is the master agent that:
- Guides users through onboarding to collect investment research preferences
...
from services.research_ai_llm_providers import get_provider, get_all_providers
from services.research_ai_tools import HYPER_AI_TOOLS
from services.research_ai_subagents import execute_subagent_tool
from services.research_ai_harness import (
    RISK_HIGH,
    TOOL_STATUS_BLOCKED,
...
SYSTEM_PROMPT_PATH = os.path.join(
    ...
    "config",
    "research_ai_system_prompt.md"
)
ONBOARDING_PROMPT_EN_PATH = os.path.join(
    os.path.dirname(os.path.dirname(__file__)),
    "config",
    "research_ai_onboarding_prompt.md"
)
ONBOARDING_PROMPT_ZH_PATH = os.path.join(
    os.path.dirname(os.path.dirname(__file__)),
    "config",
    "research_ai_onboarding_prompt_zh.md"
)
...
def load_system_prompt() -> str:
    """Load the Research AI system prompt from markdown file."""
    try:
        with open(SYSTEM_PROMPT_PATH, 'r', encoding='utf-8') as f:
            return f.read()
    except Exception as e:
        logger.error(f"Failed to load Research AI system prompt: {e}")
        return "You are AlphaTrace Research Assistant, an investment research collaborator focused on evidence-based asset and portfolio analysis."
...
DEFAULT_ONBOARDING_PROMPT_EN = """You are AlphaTrace Research Assistant, a friendly investment research assistant helping a new user get started.

Your goal is to have a natural conversation to learn about the user's investment research background, asset focus, and risk preference.

Information to collect (through natural conversation, not interrogation):
- Investment research experience level (beginner/intermediate/advanced)
- Risk preference (conservative/moderate/aggressive)
- Research focus (ETF/fund/futures/index/portfolio allocation)
- Preferred assets, sectors, indices, or horizons
...
DEFAULT_ONBOARDING_PROMPT_ZH = """你是 AlphaTrace 投研助手，一个友好的投研协作助手，正在帮助新用户入门。

你的目标是通过自然的对话了解用户的投研背景、关注资产和风险偏好。

需要收集的信息（通过自然对话，而不是审问）：
- 投研经验水平（新手/有一定经验/资深）
- 风险偏好（保守/稳健/激进）
- 研究方向（ETF/基金/期货/指数/组合配置）
- 关注资产、行业、指数或投资周期
...
def get_or_create_profile(db: Session) -> ResearchAiProfile:
...
def get_llm_config(db: Session) -> Dict[str, Any]:
    ...
    return {
        ...
        "api_key_available": bool(api_key),
        "api_format": api_format,
        "source": "legacy_research_ai_profile",
    }
...
def get_or_create_conversation(
    ...
    if conversation_id:
        conv = db.query(ResearchAiConversation).filter(
    ...
    conv = ResearchAiConversation(title="Research AI Chat", is_onboarding=is_onboarding)
    db.add(conv)
    db.commit()
    db.refresh(conv)
    return conv
...
def get_conversation_messages(
    ...
    messages = db.query(ResearchAiMessage).filter(
        ResearchAiMessage.conversation_id == conversation_id
    ).order_by(ResearchAiMessage.created_at.desc()).limit(limit).all()
...
def save_message(
    ...
    if conv:
        ...
        if role == "user" and conv.title == "Research AI Chat" and content:
...
def build_messages_for_api(
    ...
    from services.research_ai_skill_engine import (
    ...
    if user_message.startswith("/"):
        ...
        if resolved_name:
            from services.research_ai_skill_engine import load_skill
    ...
    history_orm = db.query(ResearchAiMessage).filter(
        ResearchAiMessage.conversation_id == conversation_id
    ).order_by(ResearchAiMessage.created_at).limit(100).all()
    ...
    from services.research_ai_memory_service import process_compression_memories as research_memory_processor
    result = compress_messages(messages, api_config, db=db, memory_processor=research_memory_processor)
...
def _build_profile_context(profile: ResearchAiProfile) -> str:
...
def _build_memory_context(db: Session) -> str:
    ...
    """
    from services.research_ai_memory_service import get_memories, MAX_MEMORIES
...
def _await_tool_confirmation(
    db: Session,
    assistant_msg: ResearchAiMessage,
...
def _execute_harnessed_tool_call(
    ...
    """Execute a Research AI tool with runtime harness guardrails."""
    ...
    try:
        ...
        while iteration < MAX_TOOL_ITERATIONS:
                ...
                for endpoint in endpoints:
                    try:
                        ...
                        if response.status_code == 200:
                            ...
                            logger.warning(f"[ResearchAI] Endpoint failed: {response.status_code} - {response.text[:500]}")
                    except requests.exceptions.Timeout as e:
                        last_error = f"Timeout: {str(e)}"
                        logger.warning(f"[ResearchAI] Endpoint timeout: {e}")
                    except requests.exceptions.RequestException as e:
                        last_error = str(e)
                        logger.warning(f"[ResearchAI] Request error: {e}")
            ...
            if not response or response.status_code != 200:
                ...
                logger.error(f"[ResearchAI] API failed at round {iteration}: {error_detail}")
            ...
            try:
                ...
                logger.error(f"[ResearchAI] Failed to parse response: {e}")
        ...
        try:
            ...
            if profile and profile.llm_model and conv:
                ...
                h_orm = db.query(ResearchAiMessage).filter(
                    ResearchAiMessage.conversation_id == conversation_id,
                    ResearchAiMessage.id > cp_mid
            ...
            logger.warning(f"[ResearchAI] Token calc in done event failed: {te}")

        yield format_sse_event("done", done_data)

    except Exception as e:
        logger.error(f"[ResearchAI] Error: {e}", exc_info=True)
        if tool_calls_log:
...
def start_chat_task(
    ...
    task_id = generate_task_id("research")
    manager = get_buffer_manager()
...
def _build_insight_messages(
    ...
    system_prompt = (
        "You are AlphaTrace Research Assistant inside an investment research workbench.\n"
        f"{language_instruction}"
        "You analyze market intelligence for ETF, fund, futures, index, and portfolio research users.\n"
        "Use only the provided context.\n"
        "Do not use external tools.\n"
        "Return exactly one JSON object and nothing else.\n"
        "Do not use markdown fences.\n"
        "Think in four layers before producing the JSON: technical structure, capital-flow behavior, news/event sentiment, and the conflicts between them.\n"
        "The final research view must be grounded in those layers instead of giving a free-floating opinion.\n"
        ...
        "}\n"
        "The probability must reflect research-view confidence for the next cycle and should be justified by the breakdown scores, not guessed in isolation.\n"
        "The next-cycle target and range are scenario estimates for research discussion, not trading instructions.\n"
...
def get_suggestions_context(db: Session) -> Dict[str, Any]:
    ...
    recent_convs = db.query(ResearchAiConversation).filter(
        ResearchAiConversation.is_onboarding == False,
        ResearchAiConversation.is_bot_conversation == False
    ).order_by(ResearchAiConversation.updated_at.desc()).limit(3).all()

    conversations_context = []
    for conv in recent_convs:
        # Get last 2 user messages and 2 assistant messages
        messages = db.query(ResearchAiMessage).filter(
            ResearchAiMessage.conversation_id == conv.id
        ).order_by(ResearchAiMessage.created_at.desc()).limit(4).all()
...
def build_suggestions_prompt(context: Dict[str, Any]) -> str:
    ...
    prompt_parts.append("Current Research Setup:")
    prompt_parts.append(f"- Research agent configurations: {config_status.get('trader_count', 0)}")
    prompt_parts.append(f"- Data or trigger configurations: {config_status.get('signal_pool_count', 0)}")
    prompt_parts.append(f"- Connected accounts or data credentials: {config_status.get('wallet_count', 0)}")
    prompt_parts.append("")
    ...
    prompt_parts.append("---")
    prompt_parts.append("Generate 3 short, natural investment research questions (max 30 chars each) the user might want to continue exploring.")
    ...
    prompt_parts.append("Avoid legacy automated-trading terms such as AI Trader, signal pool, wallet, exchange, leverage, open/close position.")
    prompt_parts.append('Example: ["复盘最近投研任务", "沪深300ETF风险如何", "比较两个数据源质量"]')
...
def generate_suggested_questions(db: Session) -> List[str]:
    ...
    try:
        # Use unified LLM call pattern (same as research_ai_memory_service)
        endpoints = build_chat_completion_endpoints(base_url, model)
        if api_format == "anthropic":
...
def get_or_update_suggestions(db: Session) -> Dict[str, Any]:
    ...
    conv_count = db.query(ResearchAiConversation).filter(
        ResearchAiConversation.is_onboarding == False,
        ResearchAiConversation.is_bot_conversation == False
    ).count()
