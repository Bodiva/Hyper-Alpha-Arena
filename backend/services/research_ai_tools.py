"""
Research AI Tools - Tools for Research AI main agent
...
from services.research_ai_subagents import SUBAGENT_TOOLS, execute_subagent_tool
...
def execute_analyze_tracked_address(db: Session, address: str) -> str:
    """Fetch protected Hyper Insight address detail for Research AI analysis."""
...
def execute_save_memory(
    ...
    """
    from services.research_ai_memory_service import (
...
def execute_web_search(db: Session, query: str, max_results: int = 5) -> str:
    ...
    from services.research_ai_tool_registry import get_tool_api_key

    api_key = get_tool_api_key(db, "tavily")
    if not api_key:
        return json.dumps({
            "error": "Web search is not configured. The user needs to set up their Tavily API key.",
            "setup_guide": "Go to Research AI page → right panel → Tools section → click Tavily Web Search → enter API key.",
...
def execute_get_tracked_wallets(db: Session) -> str:
    ...
    result = {
        ...
        "usage_note": "This list reflects the wallets currently synced from Hyper Insight into Hyper Alpha Arena. It is the correct source for what Research AI can currently inspect in this Arena session.",
    }
...
def _fetch_strategy_radar_universe(db: Session, *, force_refresh: bool = False) -> dict:
    ...
    if headers is None:
        return {
            "ok": False,
            "error": "Please log in to Hyper Alpha Arena before using Strategy Radar with Research AI.",
            "reason": "missing_login_token",
            "next_steps": [
                "Log in to Hyper Alpha Arena with your linked account first.",
                "After login, ask Research AI to search Strategy Radar again.",
            ],
        }
...
def execute_hyper_ai_tool(
    ...
    """Execute a Research AI tool by name."""
    try:
        ...
        # --- Skill tools: load workflow guidance (no side effects) ---
            ...
            from services.research_ai_skill_engine import load_skill_reference
        ...
        # Sub-agent tools are handled directly in research_ai_service.py main loop
