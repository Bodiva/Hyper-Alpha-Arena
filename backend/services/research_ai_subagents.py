"""
Research AI Sub-agents - Tools for calling specialized AI assistants
...
Research AI passes conversation_id to continue previous sessions.
...
# Note: Sub-agents inherit Research AI's LLM configuration, no account_id needed
SUBAGENT_TOOLS = [
    {
        "type": "function",
        "function": {
    ...
    try:
        ...
        if not llm_config.get("configured"):
            return json.dumps({
                "subagent": "prompt_ai",
                "status": "failed",
                "error": "Research AI LLM not configured."
            })
...
def execute_subagent_tool(
    ...
    The caller in research_ai_service.py uses:
