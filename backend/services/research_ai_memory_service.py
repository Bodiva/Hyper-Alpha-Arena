"""
Research AI Memory Service - User insights and memory management
...
Memory Categories:
- preference: User investment research preferences and style
- decision: Important research or allocation decisions made
- lesson: Lessons learned from research reviews or allocation outcomes
...
from database.models import ResearchAiMemory
...
# Memory categories
MEMORY_CATEGORIES = [
    "preference",  # Investment research preferences
    "decision",    # Important research/allocation decisions
...
def get_memories(
    ...
    """
    query = db.query(ResearchAiMemory)

    if active_only:
        query = query.filter(ResearchAiMemory.is_active == True)

    if category:
        query = query.filter(ResearchAiMemory.category == category)

    # Order by importance and recency
    memories = query.order_by(
        ResearchAiMemory.importance.desc(),
        ResearchAiMemory.created_at.desc()
    ).limit(limit).all()
...
) -> Optional[ResearchAiMemory]:
    """Update an existing memory."""
    memory = db.query(ResearchAiMemory).filter(ResearchAiMemory.id == memory_id).first()
    if not memory:
        return None
...
BATCH_DEDUP_PROMPT = """You are a memory deduplication assistant for an investment research AI.
...
def enforce_memory_limit(db: Session) -> int:
    ...
    """
    active_count = db.query(ResearchAiMemory).filter(
        ResearchAiMemory.is_active == True,
        ResearchAiMemory.category != "user_info"
    ).count()
    ...
    to_evict = db.query(ResearchAiMemory).filter(
        ResearchAiMemory.is_active == True,
        ResearchAiMemory.category != "user_info"
    ).order_by(
        ResearchAiMemory.importance.asc(),
        ResearchAiMemory.created_at.asc()
    ).limit(excess).all()
...
EXTRACT_MEMORIES_PROMPT = """You are a memory extraction assistant for an investment research AI platform.
...
- Research style, risk tolerance, drawdown limits, preferred investment horizon
- Preferred ETFs, funds, indices, sectors, futures, benchmarks, or portfolio constraints
- Data source preferences and evidence quality requirements

**decision** (importance 0.6-0.8):
- Research scope, asset universe, benchmark, or portfolio assumptions the user confirmed
- Data source or AgentRun configuration choices
- Important conclusions the user accepted, rejected, or asked to revisit

**lesson** (importance 0.7-0.9):
- Research mistakes, missed risks, or evidence gaps and what the user learned
- What analysis framework worked well and why
- Portfolio or market behavior patterns the user identified

**insight** (importance 0.5-0.7):
- Market observations relevant to ETFs, funds, indices, futures, sectors, or portfolio allocation
- Correlations or patterns discussed
- AgentRun/report review results and conclusions
